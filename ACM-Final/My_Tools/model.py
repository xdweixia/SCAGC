...
from My_Tools import contrastive_loss
from My_Tools import SS_Contrastive_loss
...
class GRACE(torch.nn.Module):
    def __init__(self, encoder: Encoder, num_hidden: int, num_proj_hidden: int, tau: float = 0.5):
        ...
        self.SupConLoss = SS_Contrastive_loss.Self_SupConLoss(temperature=0.1)
    ...
    def loss(self, z_i: torch.Tensor, z_j: torch.Tensor, c_i: torch.Tensor, c_j: torch.Tensor, plaels: torch.Tensor,
             epoch: int, mean: bool = True):
        h1 = z_i
        h2 = z_j
        ...
        if epoch < 0:
            ...
            embeddings = torch.cat([h1.unsqueeze(1), h2.unsqueeze(1)], dim=1)
            labels = plaels
            ret = self.SupConLoss(embeddings, labels)
            loss_instance = ret
            loss_cluster = self.criterion_cluster(c_i, c_j)
            loss = loss_instance + loss_cluster
    ...
    def forward_cluster(self, x1: torch.Tensor, edge_index1: torch.Tensor) -> torch.Tensor:
        ...
        nr = F.normalize(self.instance_projector(z), dim=1)
