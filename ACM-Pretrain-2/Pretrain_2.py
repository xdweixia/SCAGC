...
def train():
    ...
    loss = model.loss(z_i, z_j, c_i, c_j, batch_size=1024 if args.dataset == 'Coauthor-Phy' else None)
    loss.backward()
    optimizer.step()
...
def test(final=False):
    ...
    c = model.forward_cluster(data_x, data_edge_index)
    c = c.detach()
    Pred = c.cpu().detach().numpy() + 1
    # print(np.unique(GL), np.unique(Pred))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_name_Fin', type=str, default='ACM_pretrain.pkl', help='save ckpt name')
    default_param = {
    ...
    if param['drop_scheme'] == 'degree':
        ...
        feature_weights = torch.ones((data_x.size(1),)).to(device)

    log = args.verbose.split(',')

    print('Loading the best epoch')
    save_model = torch.load(args.save_name)
    ...
    for epoch in range(1, param['num_epochs'] + 1):
        ...
        # if (epoch>=50) and (epoch % 5) == 0:
            ...
            if acc > best:
                best = acc
                best_t = epoch
                torch.save(model.state_dict(), args.save_name_Fin)

            if 'eval' in log:
                print('-' * 100)
                print("Rearrange:", "\033[1;31;43m MGCN_Acc:%.4f \033[0m" % acc)
                print("Rearrange:", "\033[1;31;43m MGCN_Fsc:%.4f \033[0m" % f1_macro)
                print("Rearrange:", "\033[1;31;43m MGCN_Nmi:%.4f \033[0m" % nmi)
                print("Rearrange:", "\033[1;31;43m MGCN_Ari:%.4f \033[0m" % adjscore)
                print('-' * 100)